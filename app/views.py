...
def index(request):
    top_posts = Post.objects.all().order_by('-view_count')[0:3]  # get the three most viewed posts
    recent_posts = Post.objects.all().order_by('last_updated')[0:3]    # get the three most recently updated posts
    ...
    context = {'top_posts': top_posts, 'recent_posts' : recent_posts, 'subscribe_form' : subscribe_form, 'subscribe_successful': subscribe_successful, 'featured_blog': featured_blog, 'website_info': website_info}
...
def register_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            login(request, user) #inbuild function which takes 2 parameters, one request, another object
            return redirect("/")
...
def all_bookmarked_posts(request):
    ...
    context = {'all_bookmark_posts': all_bookmark_posts}
